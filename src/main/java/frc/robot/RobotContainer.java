...
import frc.robot.subsystems.Shoteer;
...
public class RobotContainer {
  ...
  private final Intake intake = Intake.getInstanceIntake();
  private final Shoteer shoter = Shoteer.getInstanceShoteer();
  // Controller
...
autoChooser.addOption("Auto1", new PathPlannerAuto("Auto1"));
  ...
  private void configureButtonBindings() {
    ...
    //xbox controller right bumper opens the intake and resets the angle to 0
    controller.rightBumper().onTrue(
        new InstantCommand(() -> intake.setIntakeOpen(10))).onFalse(
            new InstantCommand(() -> intake.setIntakeOpen(0)));

    //xbox controller right trigger closes the intake and resets the angle to 0
    controller.rightTrigger().whileTrue(
        new InstantCommand(() -> intake.setIntakespeed(0.5))).onFalse(
            new InstantCommand(() -> intake.setIntakespeed(0)));
  

    // xbox left trigger shoot fuel 
    controller.leftTrigger().whileTrue(
           new InstantCommand(() -> shoter.shootAtSpeed(0.5))).onFalse(
            new InstantCommand(() -> shoter.stopShoot()));
